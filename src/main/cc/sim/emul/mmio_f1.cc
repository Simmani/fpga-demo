...
void tick() {
  ...
  top->io_slave_0_aw_ready = slave->aw_ready();
  top->io_slave_0_ar_ready = slave->ar_ready();
  top->io_slave_0_w_ready = slave->w_ready();
  top->io_slave_0_b_valid = slave->b_valid();
  top->io_slave_0_b_bits_id = slave->b_id();
  top->io_slave_0_b_bits_resp = slave->b_resp();
  top->io_slave_0_r_valid = slave->r_valid();
  top->io_slave_0_r_bits_id = slave->r_id();
  top->io_slave_0_r_bits_resp = slave->r_resp();
  top->io_slave_0_r_bits_last = slave->r_last();
#if MEM_DATA_BITS > 64
  memcpy(top->io_slave_0_r_bits_data, slave->r_data(), MEM_WIDTH);
#else
  memcpy(&top->io_slave_0_r_bits_data, slave->r_data(), MEM_WIDTH);
#endif
