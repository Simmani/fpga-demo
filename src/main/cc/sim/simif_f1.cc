...
void simif_f1_t::fpga_setup() {
    ...
    check_rc(rc, (char*)"fpga_pci_init FAILED");
    ...
    check_rc(rc, (char*)"Unable to get AFI information from slot. Are you running as root?");

    /* check to see if the slot is ready */
    if (info.status != FPGA_STATUS_LOADED) {
        rc = 1;
        check_rc(rc, (char*)"AFI in Slot is not in READY state !");
    }
    ...
    if (info.spec.map[FPGA_APP_PF].vendor_id != pci_vendor_id ||
        ...
        check_rc(rc, (char*)"Unable to update PF for slot");
        /* get local image description, contains status, vendor id, and device id. */
        rc = fpga_mgmt_describe_local_image(slot_id, &info,0);
        check_rc(rc, (char*)"Unable to get AFI information from slot");
        ...
        if (info.spec.map[FPGA_APP_PF].vendor_id != pci_vendor_id ||
             info.spec.map[FPGA_APP_PF].device_id != pci_device_id) {
            rc = 1;
            check_rc(rc, (char*)"The PCI vendor id and device of the loaded AFI are not "
                             "the expected values.");
        }
    }
    ...
    check_rc(rc, (char*)"fpga_pci_attach FAILED");
