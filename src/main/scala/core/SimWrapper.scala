...
class SimReadyValidRecord(es: Seq[(String, ReadyValidIO[Data])]) extends Record {
  val elements = ListMap() ++ (es map { case (name, rv) =>
    (directionOf(rv.valid): @unchecked) match {
      case ActualDirection.Input => name -> Flipped(SimReadyValid(chiselTypeOf(rv.bits)))
      case ActualDirection.Output => name -> SimReadyValid(chiselTypeOf(rv.bits))
    }
  })
  def cloneType = new SimReadyValidRecord(es).asInstanceOf[this.type]
}

class ReadyValidTraceRecord(val es: Seq[(String, ReadyValidIO[Data])]) extends Record {
  val elements = ListMap() ++ (es map {
    case (name, rv) => name -> ReadyValidTrace(chiselTypeOf(rv.bits))
  })
  def cloneType = new ReadyValidTraceRecord(es).asInstanceOf[this.type]
}

class SimWrapperIO(io: TargetBoxIO)(implicit val p: Parameters)
    extends Bundle with HasSimWrapperParams {
  ...
  /*** Instrumentation ***/
  val cntrs = p(ToggleCounterSize) map (size => Decoupled(Output(Vec(size, UInt(32.W)))))
  val daisy = new strober.core.DaisyBundle(daisyWidth)
  val traceLen = Input(UInt(log2Ceil(traceMaxLen + 1).W))
  val wireInTraces = 
  ...
  override def cloneType: this.type = new SimWrapperIO(io).asInstanceOf[this.type]
}

class TargetBoxIO(targetIo: Seq[Data]) extends Record {
  val elements = ListMap() ++ (targetIo map (port => port.instanceName -> chiselTypeOf(port)))
...
class SimWrapper(targetIo: Seq[Data])
  ...
  def genReadyValidChannel[T <: Data](arg: (String, ReadyValidIO[T])) =
    arg match { case (name, io) =>
      val channel = Module(new ReadyValidChannel(
        chiselTypeOf(io.bits), directionOf(io.valid) == ActualDirection.Input))
  ...
  val cntrReady = io.cntrs match {
    case None => true.B
    case Some(port) =>
      val (readys, valids) = (port.bits.zipWithIndex map { case (cntr, i) =>
        val channel = Module(new Queue(chiselTypeOf(cntr), p(ChannelLen)))
        channel suggestName s"CounterChannel_$i"
        channel.io.enq.bits := DontCare
         // Show the counter value after it's updated
        channel.io.enq.valid := RegNext(fire) || resetNext
        channel.io.deq.ready := port.ready
        cntr := channel.io.deq.bits
        (channel.io.enq.ready, channel.io.deq.valid)
      }).unzip
      port.valid := Cat(valids).andR
      Cat(readys).andR
  }
