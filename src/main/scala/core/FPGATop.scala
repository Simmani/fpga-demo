...
class FPGATopIO(implicit val p: Parameters) extends Bundle {
...
class FPGATop(simIoType: SimWrapperIO)(implicit p: Parameters) extends Module with HasWidgets {
  ...
  master.io.hostReset := reset.toBool
  ...
  defaultIOWidget.io.hostReset := reset.toBool
  ...
  if (p(EnableSnapshot)) {
    ...
    daisyController.io.hostReset := reset.toBool
    ...
    traceWidget.io.hostReset := reset.toBool
  ...
  if (p(MemModelKey) != None) {
    ...
    loadMem.io.hostReset := reset.toBool
  ...
  defaultIOWidget.io.tReset.ready := ((simIo.cntrs foldLeft true.B){ (resetReady, cntrs) =>
    val widget = addWidget(
      new dessert.widgets.ToggleCounterWidget(cntrs.bits.size),
      "ToggleCounterWidget")
    widget.reset := reset.toBool || simReset
    widget.io.hostReset := reset.toBool
    widget.io.counters <> cntrs
    createResetQueue(widget.io.tReset)
  }) && (simIo.endpoints foldLeft true.B){ (resetReady, endpoint) =>
    ((0 until endpoint.size) foldLeft resetReady){ (ready, i) =>
      ...
      widget match {
        case model: MemModel =>
          arb.io.master(i) <> model.io.hostMem
