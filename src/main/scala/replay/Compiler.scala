...
class Emitter(dir: File) extends VerilogEmitter {
...
class Compiler(dir: File) extends firrtl.Compiler {
  override def emitter = new Emitter(dir)
...
object Compiler {
  def apply(
    ...
    val state   = (new LowFirrtlCompiler).compile(
      ...
      dessert.passes.CodeOptimizations,
      new dessert.passes.SVFBackAnnotationPass))
    val result  = (new Compiler(dir)).compileAndEmit(state, Nil)
