...
class PeekPokeIOWidgetIO(val inNum: Int, val outNum: Int)(implicit p: Parameters) extends WidgetIO {
  // Channel width == width of simulation MMIO bus
  val ins = // Vec(inNum, Decoupled(UInt(ctrl.nastiXDataBits.W)))
    if (inNum > 0) Vec(inNum, Decoupled(UInt(ctrl.nastiXDataBits.W)))
    else Input(Vec(inNum, Decoupled(UInt(ctrl.nastiXDataBits.W))))
  val outs = // Flipped(Vec(outNum, Decoupled(UInt(ctrl.nastiXDataBits.W))))
    if (outNum > 0) Flipped(Vec(outNum, Decoupled(UInt(ctrl.nastiXDataBits.W))))
    else Output(Vec(outNum, Decoupled(UInt(ctrl.nastiXDataBits.W))))
...
class PeekPokeIOWidget(inputs: Seq[(String, Int)], outputs: Seq[(String, Int)])
  ...
  def bindInputs = bindChannels((name, offset) => {
    ...
    val reg = Reg(chiselTypeOf(channel.bits))
