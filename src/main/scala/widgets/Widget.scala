...
abstract class WidgetIO(implicit val p: Parameters) extends Bundle {
...
abstract class Widget(implicit p: Parameters) extends Module {
  ...
  def genCRFile() {
    val crFile = Module(new MCRFile(this.getClass.getSimpleName, numRegs)(
      p alterPartial ({ case NastiKey => p(CtrlNastiKey) })))
    crFile.reset := io.hostReset
