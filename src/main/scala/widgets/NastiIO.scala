...
abstract class NastiWidgetBase(implicit p: Parameters) extends MemModel {
  ...
  def elaborate(stall: Bool,
    ...
    val fire = tFire && !stall && arBuf.io.enq.ready &&
               awBuf.io.enq.ready && wBuf.io.enq.ready
    fire suggestName "fire"
    io.tNasti.toHost.hReady := fire
    io.tNasti.fromHost.hValid := fire || RegNext(reset.toBool)
    ...
    // Request
    tNasti.ar.ready    := rCycleReady
    tNasti.aw.ready    := wCycleReady
    tNasti.w.ready     := wCycleReady
...
class NastiWidget(implicit val p: Parameters) extends NastiWidgetBase {
  ...
  // Disable hostMem
  io.hostMem.ar.valid := false.B
  io.hostMem.aw.valid := false.B
  io.hostMem.w.valid := false.B
  io.hostMem.r.ready := false.B
  io.hostMem.b.ready := false.B
