...
class MCRIO(wName: String, numCRs: Int)(implicit p: Parameters) extends NastiBundle()(p) {
  ...
  def bindReg(reg: RegisterEntry, addr: Int): Unit = {
    if (reg.permissions.writeable) {
      ...
      assert(!write(addr).valid, s"[widget: ${wName}] Register ${reg.name} is read only")
    }

    if (reg.permissions.readable) {
      read(addr).bits := reg.node
    } else {
      assert(!read(addr).ready, "[widget: ${wName} Register ${reg.name} is write only")
    }
  ...
  def bindDecoupledSink(channel: DecoupledSinkEntry, addr: Int): Unit = {
    ...
    assert(!read(addr).ready, s"[widget: ${wName}, addr: ${addr}] Can only write to this decoupled sink")
  }

  def bindDecoupledSource(channel: DecoupledSourceEntry, addr: Int): Unit = {
    read(addr) <> channel.node
    assert(!write(addr).valid, s"[widget: ${wName}, addr: ${addr}] Can only read from this decoupled source")
  }

}

class MCRFile(wName: String, numRegs: Int)(implicit p: Parameters) extends NastiModule()(p) {
  val io = IO(new Bundle {
    val nasti = Flipped(new NastiIO)
    val mcr = new MCRIO(wName, numRegs)
  })

  val depth = log2Ceil(numRegs) max 1
  val wordAddrOffset = log2Ceil(nastiWStrobeBits) max 1
  ...
  val rAddr = Reg(UInt(depth.W))
  val rData = Reg(UInt(nastiXDataBits.W))
  val wAddr = Reg(UInt(depth.W))
  ...
  when(io.nasti.aw.fire()){
    awFired := true.B
    wAddr := (io.nasti.aw.bits.addr >> wordAddrOffset)
  ...
  when(io.nasti.ar.fire()) {
    arFired := true.B
    rAddr := (io.nasti.ar.bits.addr >> wordAddrOffset)
    rId := io.nasti.ar.bits.id
    assert(io.nasti.ar.bits.len === 0.U,
      "MCRFile only support single beat reads")
  }
  ...
  io.mcr.write foreach { w =>
    w.valid := false.B
    w.bits := wData
  }

  io.mcr.read foreach { r =>
    r.ready := false.B
  }
  ...
  io.nasti.ar.ready := ~arFired && ~reset.toBool
  io.nasti.aw.ready := ~awFired && ~reset.toBool
  io.nasti.w.ready := ~wFired && ~reset.toBool
}
